...
def read_data_from_json_files(paths: List[str], upsample_rates: List = None) -> List:
    ...
    for i, path in enumerate(paths):
        with open(path, "r", encoding="utf-8") as f:
            ...
            curr_mem = torch.cuda.max_memory_allocated() / 1024**2
            rem = total_mem - curr_mem

            print("Available Memory NOWWWWW: ",rem)
            logger.info("Reading file %s" % path)
            data = json.load(f)
            print("DATA TYPESSSS: ",type(data))
            upsample_factor = int(upsample_rates[i])
            data = data * upsample_factor
            random.shuffle(data)
            #return results[:len(results)//6]
            results.extend(data[:len(data)//96])
            props = torch.cuda.get_device_properties(device)
            total_mem = props.total_memory / 1024**2
            curr_mem = torch.cuda.max_memory_allocated() / 1024**2
            rem = total_mem - curr_mem
